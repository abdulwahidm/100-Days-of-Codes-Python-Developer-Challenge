...
A function fv() was implemented to calculate the future value of investment 
depending on the number of capitalization periods during the year 
(m parameter, default 1):

def fv(pv, rate, n, m=1):
    return round(pv * (1 + (rate / m)) ** (n * m), 2)

The annual_acc_factor() function was created, 
which calculates the annual accumulation factor:

annual_acc_factor = partial(fv, n=1, pv=1)

Create two more functions that calculates the value of the annual accumulation factor 
for the following capitalization models:
